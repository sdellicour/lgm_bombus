# NOTES:
	# - idea: re-performing the analyses with only two or three uncorrolated temperature/precipitation variables?
	# - the current "CSI SRI difference" maps do not make a lot of sense as they do not consider the LGM ice sheet
...
timePeriods = c("Present time","Mid-Holocene","Last GLM")
timePeriodIDs = c("Present_t0","Past_Holo","Past_GLM")
...
if (!file.exists(paste0("All_SIppc_values.csv")))
	{
		...
		pdf(paste0("All_the_figures_&_SI/SIppc_species_curves_NEW.pdf"), width=10, height=12)
...
rasters_CSI_list_is = list() # rasters_CSI_list without the IC
rasters_SRI_list_is = list() # rasters_SRI_list without the IC
for (j in 1:length(models))
	{
		...
		for (i in 1:dim(species)[1])
			{
				bufferRasters[[i]] = projections1[[i]][[j]][[2]]
				txt = SIppcs[i,"optimisedThreshold"]
				cutOff = as.numeric(unlist(strsplit(txt," \\["))[1])
				c1 = projections1[[i]][[j]][[2]]; c1[c1[]>cutOff] = 1
				counterRasters[[i]] = c1
			}
		...
		rasters_CSI[[3]] = mean(stack(bufferRasters)); rasters_SRI[[3]] = sum(stack(counterRasters))
		rasters_CSI_list[[j]] = rasters_CSI; rasters_SRI_list[[j]] = rasters_SRI
		rasters_CSI_minus_IC = list(); rasters_SRI_minus_IC = list()
		rasters_CSI_minus_IC[[1]] = rasters_CSI[[1]]; rasters_CSI_minus_IC[[2]] = rasters_CSI[[2]]; rasters_CSI_minus_IC[[3]] = rasters_CSI[[3]]
		rasters_SRI_minus_IC[[1]] = rasters_SRI[[1]]; rasters_SRI_minus_IC[[2]] = rasters_SRI[[2]]; rasters_SRI_minus_IC[[3]] = rasters_SRI[[3]]
		rasters_CSI_minus_IC[[1]][raster::extract(rasters_CSI_minus_IC[[1]],is_t0_0m,cellnumbers=T)[[1]][,"cell"]] = 0
		rasters_CSI_minus_IC[[2]][raster::extract(rasters_CSI_minus_IC[[2]],is_Holo_0m,cellnumbers=T)[[1]][,"cell"]] = 0
		rasters_CSI_minus_IC[[3]][raster::extract(rasters_CSI_minus_IC[[3]],is_LGM_0m,cellnumbers=T)[[1]][,"cell"]] = 0
		rasters_SRI_minus_IC[[1]][raster::extract(rasters_SRI_minus_IC[[1]],is_t0_0m,cellnumbers=T)[[1]][,"cell"]] = 0
		rasters_SRI_minus_IC[[2]][raster::extract(rasters_SRI_minus_IC[[2]],is_Holo_0m,cellnumbers=T)[[1]][,"cell"]] = 0
		rasters_SRI_minus_IC[[3]][raster::extract(rasters_SRI_minus_IC[[3]],is_LGM_0m,cellnumbers=T)[[1]][,"cell"]] = 0			
		rasters_CSI_list_is[[j]] = rasters_CSI_minus_IC; rasters_SRI_list_is[[j]] = rasters_SRI_minus_IC
	}
if (savingPlots)
	{
		...
		pdf(paste0("All_the_figures_&_SI/CSI_maps_LGM_t0_1_NEW.pdf"), width=7.5, height=10.5)
		par(mfrow=c(3,3), oma=c(1.0,2.0,1.0,0.0), mar=c(0.0,0.0,0.0,0.2), lwd=0.2, col="gray30", col.axis="gray30")
		for (j in c(3,1))
			{
				for (i in 1:length(rasters_CSI_list))
					{
						...
						rect(-12, 67, 5, 71.5, lwd=0.2, col="white", border=NA)
						if (j == 1) mtext(paste0(timePeriods[j]), at=-10, line=-3.8, adj=0, cex=0.55, col="gray30")
						if (j == 3) mtext(paste0(timePeriods[j],"(",model_names[i],")"), at=-10, line=-3.8, adj=0, cex=0.55, col="gray30")
		...
		plot(rastLegend, col=colour_scale_1, legend.only=T, add=T, legend.width=0.5, legend.shrink=0.3, smallplot=c(0.00,0.02,0.037,0.963), adj=3,
			 ...
			 mgp=c(0,0.6,0)), alpha=1, side=3, horizontal=F)
		dev.off()
		pdf(paste0("All_the_figures_&_SI/SRI_maps_LGM_t0_1_NEW.pdf"), width=7.5, height=10.5)
		...
		plot(rastLegend, col=colour_scale_2, legend.only=T, add=T, legend.width=0.5, legend.shrink=0.3, smallplot=c(0.00,0.02,0.037,0.963), adj=3,
			 ...
			 mgp=c(0,0.6,0)), alpha=1, side=3, horizontal=F)
		dev.off()
		pdf(paste0("All_the_figures_&_SI/CSI_maps_LGM_t0_2a_NEW.pdf"), width=7.5, height=10.5)
		par(mfrow=c(3,3), oma=c(1.0,2.0,1.0,0.0), mar=c(0.0,0.0,0.0,0.2), lwd=0.2, col="gray30", col.axis="gray30")
		for (i in 1:length(rasters_CSI_list))
			{
				if (i == 1) indices = 3:1
				...
				for (j in indices)
					{
						...
						if (j == 2)
							{
								plot(is_Holo_0m, add=T, border="gray50", lwd=0.1, col=rgb(255,255,255,255,maxColorValue=255))
								plot(is_Holo_0m, add=T, border="gray50", lwd=0.1, col=rgb(77,77,77,40,maxColorValue=255)) # "#F7FBFF"
							}
						if (j == 1)
							{
								plot(is_t0_0m, add=T, border="gray50", lwd=0.1, col=rgb(255,255,255,255,maxColorValue=255))
						...
						mtext(timePeriods[j], at=-10, line=-2.8, adj=0, cex=0.55, col="gray30")
						if (j != 1) mtext(paste0("(",model_names[i],")"), at=-10, line=-3.8, adj=0, cex=0.55, col="gray30")
						rect(-13, 35, 29, 72, lwd=0.2, col=NA, border="gray30")
					}
				if (i == 2)
					{
		...
		dev.off()
		pdf(paste0("All_the_figures_&_SI/SRI_maps_LGM_t0_2a_NEW.pdf"), width=7.5, height=10.5)
		par(mfrow=c(3,3), oma=c(1.0,2.0,1.0,0.0), mar=c(0.0,0.0,0.0,0.2), lwd=0.2, col="gray30", col.axis="gray30")
		for (i in 1:length(rasters_SRI_list))
			{
				if (i == 1) indices = 3:1
		...
		pdf(paste0("All_the_figures_&_SI/CSI_maps_LGM_t0_2b_NEW.pdf"), width=4.5, height=3.5); plottingQuantiles = FALSE
		par(mfrow=c(3,1), oma=c(0,0,0,0), mar=c(2,2,1,0), lwd=0.2, col="gray30", col.axis="gray30")	
		for (i in 1:length(rasters_CSI_list))
			{
				for (j in 1:3)
					{
						rast = rasters_CSI_list_is[[i]][[j]]
						# rast[rast[]==0] = NA # to do not account for the ice sheet
						mat = as.matrix(rast)
						if (j == 1)
							{
								medianValues = matrix(nrow=dim(mat)[1], ncol=4); medianValues[,1] = yFromRow(rast, 1:dim(mat)[1])
								qt0025Values = matrix(nrow=dim(mat)[1], ncol=4); qt0025Values[,1] = yFromRow(rast, 1:dim(mat)[1])
								qt0975Values = matrix(nrow=dim(mat)[1], ncol=4); qt0975Values[,1] = yFromRow(rast, 1:dim(mat)[1])
							}
						for (k in 1:dim(mat)[1])
							{
								medianValues[k,1+j] = median(mat[k,], na.rm=T)
								qt0025Values[k,1+j] = quantile(mat[k,], 0.025, na.rm=T)
								qt0975Values[k,1+j] = quantile(mat[k,], 0.975, na.rm=T)
							}
					}
				plot(medianValues[,1], medianValues[,2], ylim=c(0.2,0.5), type="l", col=NA, axes=F, ann=F)
				if (plottingQuantiles)
					{
						for (j in 1:3)
							{
								lines(medianValues[,1], qt0025Values[,j+1], type="l", lwd=0.2, lty=2, col=cols1[[j]])
								lines(medianValues[,1], qt0025Values[,j+1], type="l", lwd=0.2, lty=2, col=cols1[[j]])
							}
					}
				for (j in 1:3)
					{
						lines(medianValues[,1], medianValues[,j+1], type="l", lwd=0.8, lty=1, col=cols1[[j]])
					}
				minX = min(medianValues[,1]); maxX = max(medianValues[,1])
				axis(side=1, lwd.tick=0.2, cex.axis=0.9, lwd=0.2, tck=-0.05, col.axis="gray30", mgp=c(0,0.25,0), at=c(minX,maxX))
				axis(side=2, lwd.tick=0.2, cex.axis=0.9, lwd=0.2, tck=-0.05, col.axis="gray30", mgp=c(0,0.3,0))
			}	
		dev.off()
		pdf(paste0("All_the_figures_&_SI/SRI_maps_LGM_t0_2b_NEW.pdf"), width=4.5, height=3.5); plottingQuantiles = FALSE
		par(mfrow=c(3,1), oma=c(0,0,0,0), mar=c(2,2,1,0), lwd=0.2, col="gray30", col.axis="gray30")	
		for (i in 1:length(rasters_SRI_list))
			{
				for (j in 1:3)
					{
						rast = rasters_SRI_list_is[[i]][[j]]
						# rast[rast[]==0] = NA # to do not account for the ice sheet
						mat = as.matrix(rast)
						if (j == 1)
							{
				...
				plot(medianValues[,1], medianValues[,2], ylim=c(10,30), type="l", col=NA, axes=F, ann=F)
				if (plottingQuantiles)
					{
						for (j in 1:3)
							{
...
if (savingPlots)
	{
		...
		colour_scale_2 = rev(colorRampPalette(brewer.pal(11,"RdBu"))(120))[10:110]
		minV1 = Inf; maxV1 = -Inf; minV2 = Inf; maxV2 = -Inf
		for (i in 1:length(rasters_CSI_list_is))
			{
				if (minV1 > min(rasters_CSI_list[[i]][[1]][]-rasters_CSI_list[[i]][[3]][],na.rm=T)) minV1 = min(rasters_CSI_list[[i]][[1]][]-rasters_CSI_list[[i]][[3]][],na.rm=T)
				if (maxV1 < max(rasters_CSI_list[[i]][[1]][]-rasters_CSI_list[[i]][[3]][],na.rm=T)) maxV1 = max(rasters_CSI_list[[i]][[1]][]-rasters_CSI_list[[i]][[3]][],na.rm=T)
			}
		for (i in 1:length(rasters_SRI_list_is))
			{
				if (minV2 > min(rasters_SRI_list[[i]][[1]][]-rasters_SRI_list[[i]][[3]][],na.rm=T)) minV2 = min(rasters_SRI_list[[i]][[1]][]-rasters_SRI_list[[i]][[3]][],na.rm=T)
				if (maxV2 < max(rasters_SRI_list[[i]][[1]][]-rasters_SRI_list[[i]][[3]][],na.rm=T)) maxV2 = max(rasters_SRI_list[[i]][[1]][]-rasters_SRI_list[[i]][[3]][],na.rm=T)
			}
		...
		for (i in 1:length(rasters_CSI_list))
			{
				rast = rasters_CSI_list[[i]][[1]]-rasters_CSI_list[[i]][[3]]
				buffer = rast; buffer[!is.na(buffer[])] = 1; buffer[is.na(buffer[])] = 0
				contour = rasterToContour(buffer, maxpixels=100000)
				index1 = round(((min(rast[],na.rm=T)-minV1)/(maxV1-minV1))*100)+1
				index2 = round(((max(rast[],na.rm=T)-minV1)/(maxV1-minV1))*100)+1
				...
				plot(contour, col="gray50", lwd=0.1, add=T)
				plot(is_t0_0m, add=T, border="gray50", lwd=0.1, col=rgb(255,255,255,255,maxColorValue=255))
				plot(is_t0_0m, add=T, border="gray50", lwd=0.1, col=rgb(77,77,77,40,maxColorValue=255)) # "#F7FBFF"
				rect(-12, 67, 5, 71.5, lwd=0.2, col="white", border=NA)
				mtext("CSI diff. (t0-LGM)", at=-10, line=-2.8, adj=0, cex=0.55, col="gray30")
				mtext(paste0("(",model_names[i],")"), at=-10, line=-3.8, adj=0, cex=0.55, col="gray30")
				rect(-13, 35, 29, 72, lwd=0.2, col=NA, border="gray30")
			}
		for (i in 1:length(rasters_SRI_list))
			{
				rast = rasters_SRI_list[[i]][[1]]-rasters_SRI_list[[i]][[3]]
				buffer = rast; buffer[!is.na(buffer[])] = 1; buffer[is.na(buffer[])] = 0
				contour = rasterToContour(buffer, maxpixels=100000)
				index1 = round(((min(rast[],na.rm=T)-minV2)/(maxV2-minV2))*100)+1
				index2 = round(((max(rast[],na.rm=T)-minV2)/(maxV2-minV2))*100)+1
				...
				plot(is_t0_0m, add=T, border="gray50", lwd=0.1, col=rgb(77,77,77,40,maxColorValue=255)) # "#F7FBFF"
				rect(-12, 67, 5, 71.5, lwd=0.2, col="white", border=NA)
				mtext("SRI diff. (t0-LGM)", at=-10, line=-2.8, adj=0, cex=0.55, col="gray30")
				mtext(paste0("(",model_names[i],")"), at=-10, line=-3.8, adj=0, cex=0.55, col="gray30")
				rect(-13, 35, 29, 72, lwd=0.2, col=NA, border="gray30")
			}		
		rast = projections1[[i]][[1]][[1]]; rast[] = NA
